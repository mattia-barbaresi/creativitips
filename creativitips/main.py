            ...
            for interf in interferences:
                for forg in forgets:
                    for t_mem in thresholds_mem:
                        # read input model
                        in_path = const.OUT_DIR + "tps_results_" + str(const.STM_DECAY_RATE) + "_" + str(const.LTM_DECAY_RATE) \
                                  + "/" + utils.params_to_string(method, order, forg, interf, t_mem) \
                                  + file_name + "/tps_units.dot"
                        in_gpath = const.OUT_DIR + "tps_results_" + str(const.STM_DECAY_RATE) + "_" + str(const.LTM_DECAY_RATE) \
                                   + "/" + utils.params_to_string(method, order, forg, interf, t_mem) \
                                   + file_name + "/ggraph.dot"
                        # ============= GRAPH
                        ...
                        os.makedirs(const.OUT_DIR + "/creativeGens/", exist_ok=True)
                        ct.plot_nx_creativity(GG, const.OUT_DIR + "creativeGens/graph")
                        gens_out, goid = ct.creative_ggens(rng, GG, n_seq=10, min_len=100)
                        with open(const.OUT_DIR + "creativeGens/generated.json", "w") as fp:
                            json.dump(gens_out, fp)
                        with open(const.OUT_DIR + "creativeGens/gens_data.json", "w") as fp:
