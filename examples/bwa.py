    ...
    for i, sequence_val in enumerate(test_sequence):
        for j in range(len(states)):
            ...
            # else perform this
                ...
                values = [node_values_fwd[k, i - 1] * emission[j, sequence_syms[sequence_val]] * transition[k, j] for k
                          in
                          range(len(states))]
    ...
    for i in range(1, len(test_sequence) + 1):
        for j in range(len(states)):
            ...
            # else perform this
                ...
                values = [
                    node_values_bwd[k, -i + 1] * emission[k, sequence_syms[test_sequence[-i + 1]]] * transition[j, k]
